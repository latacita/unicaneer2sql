...
import es.unican.moses.sle.db.eer2sql.diagram.custom.commands.CreateCompositeAttributeinRelationshipCommand;
import es.unican.moses.sle.db.eer2sql.diagram.custom.commands.CreateSingleAttributeinRelationshipCommand;
import es.unican.moses.sle.db.eer2sql.diagram.custom.commands.DeleteCompositeAttributeinRelationshipCommand;
import es.unican.moses.sle.db.eer2sql.diagram.custom.commands.DeleteSingleAttributeinRelationshipCommand;
...
public class RelationshipWizard extends JFrame {
	 ...
	 * @param editpart
	 */
	public RelationshipWizard(EditPart editpart){
		super();
		//access to model
	...
	private void updateAttributesList(){
		String [] attr = new String[element.getAttributes().size()];
		for(int i=0; i<element.getAttributes().size(); i++){
			attr[i] = element.getAttributes().get(i).getName();
		}
	...
	private void updateAttributesValues(){
		...
		if(element.getAttributes().size() > 0){
			...
			if(element.getAttributes().get(index).eClass().getName().equalsIgnoreCase("SingleAttribute")){
				//Create a single attribute
				SingleAttribute a = (SingleAttribute) element.getAttributes().get(index);
	...
	private void updateValueofAttribute(){
		...
		if(element.getAttributes().get(index).eClass().getName().equalsIgnoreCase("SingleAttribute")){
			...
			//invoke the command
			UpdateSingleAttributeinRelationshipCommand cmd = 
				new UpdateSingleAttributeinRelationshipCommand(TEDomain, element,
			...
			//invoke the command
			UpdateCompositeAttributeinRelationshipCommand cmd = 
				new UpdateCompositeAttributeinRelationshipCommand(TEDomain, element,
						name, nullable, index);
			try {
				cmd.execute(new NullProgressMonitor(), null);
			} catch (org.eclipse.core.commands.ExecutionException e) {
	...
	private void deleteAttribute(){
		...
		if(element.getAttributes().size() != 0){
			indexList = attributesList.getSelectedIndex();
			//if is a single attribute 
			if(element.getAttributes().get(indexList).eClass().getName().equalsIgnoreCase("SingleAttribute")){
				//invoke the command
				DeleteSingleAttributeinRelationshipCommand cmd = 
					new DeleteSingleAttributeinRelationshipCommand(TEDomain, element,
							diagram, indexList);
				try {
					cmd.execute(new NullProgressMonitor(), null);
				...
				DeleteCompositeAttributeinRelationshipCommand cmd = 
					new DeleteCompositeAttributeinRelationshipCommand(TEDomain, element,
							diagram, indexList);
				try {
					cmd.execute(new NullProgressMonitor(), null);
				} catch (org.eclipse.core.commands.ExecutionException ee) {
	...
	private void addNewCompositeAttribute(){
		indexList = attributesList.getModel().getSize();
		//invoke the command
		CreateCompositeAttributeinRelationshipCommand cmd = 
			new CreateCompositeAttributeinRelationshipCommand(TEDomain, element,
