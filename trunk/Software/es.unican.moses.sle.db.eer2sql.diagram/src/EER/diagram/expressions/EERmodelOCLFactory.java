...
public class EERmodelOCLFactory {
	...
	protected EERmodelOCLFactory() {
		this.expressions = new EERmodelAbstractExpression[14];
	}
	...
	public static EERmodelAbstractExpression getExpression(int index,
		...
		if (cached.expressions[index] == null) {
			final String[] exprBodies = new String[] {
					...
					"self.singlesAttributes -> notEmpty()", //$NON-NLS-1$
					"self.participants -> forAll(p1, p2 |  p1.target.oclIsKindOf(WeakEntity) implies p2.target.oclIsKindOf(Entity))", //$NON-NLS-1$
					"self.name.size() > 0", //$NON-NLS-1$
					"self.name.size() > 0", //$NON-NLS-1$
					"self.domain <> null", //$NON-NLS-1$
					"self.upperBound >= self.lowerBound ", //$NON-NLS-1$
			};
